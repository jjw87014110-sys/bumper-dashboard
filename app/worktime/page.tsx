...
export default function WorktimePage() {
  ...
  const [workHoursEdits, setWorkHoursEdits] = useState<Record<string, string>>({})
  ...
  // 근무시간(h) 직접 수정 — 관리자 수동 보정 (0.5h 단위)
  async function saveWorkHours(staffName: string, date: string, hours: number) {
    const safe = Math.max(0, Math.round(hours * 2) / 2) // 0.5h 단위 보정
    const { error } = await supabase
      .from('worktime_records')
      .update({ work_hours: safe })
      .eq('staff_name', staffName)
      .eq('date', date)
    if (error) { showToast('근무시간 저장 실패', 'error'); return }
    showToast('근무시간 저장됨')
    logAudit(getCurrentUserName(), 'UPDATE', 'worktime_records', `${staffName} ${date} 근무시간 ${safe}h`)
    fetchAll()
  }
  ...
  async function handleUpload() {
    ...
    for (const file of files) {
      try {
        ...
        // 잔업(overtime_minutes)·근무시간(work_hours)은 관리자 수동 보정 가능 값이므로
        // ERP 재업로드 시 기존에 저장된 날짜는 보존하고 신규 날짜만 ERP값 사용
        const existing = new Set(
          allRecords.map(r => `${r.staff_name}|${r.date}`)
        )
        const recordsToUpsert = records.map(rec => {
          const { overtime_minutes, ...rest } = rec
          if (existing.has(`${rec.staff_name}|${rec.date}`)) {
            const { work_hours, ...noHours } = rest
            return noHours
          }
          return rest
        })
        const { error } = await supabase
          .from('worktime_records')
          .upsert(recordsToUpsert, { onConflict: 'staff_name,date' })
        if (error) {
