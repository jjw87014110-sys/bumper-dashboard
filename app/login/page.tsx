...
export default function LoginPage() {
      ...
      <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg-base)' }}>
        <div style={{ width:360 }}>
          <div style={{ textAlign:'center', marginBottom:28 }}>
            ...
            <div style={{ fontSize:12, color:'var(--text-muted)', marginBottom:8 }}>허가된 네트워크 외 접속입니다.</div>
          ...
          <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:14, padding:24 }}>
            ...
            <form onSubmit={handleExternalPw}>
              <input className="form-input" type="password" placeholder="외부 접속 비밀번호"
                value={externalPw} onChange={e => setExternalPw(e.target.value)}
                autoFocus style={{ marginBottom:12 }} />
              {externalError && (
                <div style={{ background:'var(--accent-red-dim)', border:'1px solid var(--accent-red)', borderRadius:7, padding:'8px 12px', fontSize:12, color:'var(--accent-red)', marginBottom:12 }}>
                  {externalError}
                </div>
              )}
  ...
  // 정상 로그인
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg-base)', backgroundImage:'radial-gradient(ellipse at 20% 50%, rgba(59,126,248,0.08) 0%, transparent 60%)' }}>
      <div style={{ width:360 }}>
        ...
        <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:14, padding:28 }}>
          {step === 'login' ? (
            <form onSubmit={handleLogin}>
              ...
              {error && <div style={{ background:'var(--accent-red-dim)', border:'1px solid var(--accent-red)', borderRadius:7, padding:'8px 12px', fontSize:12, color:'var(--accent-red)', marginBottom:14 }}>{error}</div>}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:16 }}>
                {[1,2,3,4,5,6,7,8,9].map(n => (
                  <button key={n} type="button" onClick={() => handlePinInput(String(n))}
                    style={{ padding:'14px', fontSize:18, fontWeight:600, background:'var(--bg-hover)', border:'1px solid var(--border)', borderRadius:10, cursor:'pointer', color:'var(--text-primary)' }}
                ...
                <button type="button" onClick={() => setPin('')} style={{ padding:'14px', fontSize:12, background:'var(--bg-hover)', border:'1px solid var(--border)', borderRadius:10, cursor:'pointer', color:'var(--accent-red)' }}>지우기</button>
                <button type="button" onClick={() => handlePinInput('0')}
                  style={{ padding:'14px', fontSize:18, fontWeight:600, background:'var(--bg-hover)', border:'1px solid var(--border)', borderRadius:10, cursor:'pointer', color:'var(--text-primary)' }}
                  onMouseEnter={e => (e.currentTarget as HTMLElement).style.background='var(--accent-blue-dim)'}
                  onMouseLeave={e => (e.currentTarget as HTMLElement).style.background='var(--bg-hover)'}
                >0</button>
                <button type="button" onClick={() => setPin(prev => prev.slice(0,-1))} style={{ padding:'14px', fontSize:16, background:'var(--bg-hover)', border:'1px solid var(--border)', borderRadius:10, cursor:'pointer', color:'var(--text-secondary)' }}>←</button>
              </div>
              {error && <div style={{ background:'var(--accent-red-dim)', border:'1px solid var(--accent-red)', borderRadius:7, padding:'8px 12px', fontSize:12, color:'var(--accent-red)', marginBottom:14, textAlign:'center' }}>{error}</div>}
              <button className="btn btn-primary" type="submit" disabled={loading||pin.length!==4} style={{ width:'100%', justifyContent:'center', padding:'10px', fontSize:13, opacity:pin.length!==4?0.5:1 }}>
                {loading ? '확인 중...' : 'PIN 확인'}
              </button>
              <button type="button" onClick={() => { setStep('login'); setPin(''); setError(''); setAttempts(0) }} style={{ width:'100%', marginTop:10, padding:'8px', fontSize:12, background:'transparent', border:'none', color:'var(--text-muted)', cursor:'pointer' }}>
                ← 로그인으로 돌아가기
              </button>
            </form>
          )}
