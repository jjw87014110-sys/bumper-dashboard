                  ...
                  {loading ? (
                    ...
                    <div style={{ overflowX: 'auto' }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        ...
                        <tbody>{data.map((r,i) => <tr key={i}><td style={td}>{r.date}</td><td style={td}>{r.time_of_day||'-'}</td><td style={td}><span className="badge badge-gray">{r.model||'-'}</span></td><td style={td}>{r.category||'-'}</td><td style={td}>{r.scratch_location||'-'}</td><td style={td}><span className={`badge ${r.jig_status==='양호'?'badge-green':'badge-red'}`}>{r.jig_status||'-'}</span></td><td style={td}><span className={`badge ${r.equipment_issue==='해당없음'?'badge-gray':'badge-amber'}`}>{r.equipment_issue||'-'}</span></td><td style={td}>{r.action||'-'}</td><td style={td}>{r.note||'-'}</td></tr>)}</tbody>
                      </table>
                    </div>
                  )}
                </div>
