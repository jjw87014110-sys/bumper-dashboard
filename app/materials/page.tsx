...
export default function Page() {
  ...
  async function selectEquipment(eq: any) {
    setSelected(eq)
    setLoading(true)
    const { data } = await supabase.from('materials').select('*').eq('equipment_no', eq.no).order('item_no', { ascending: false })
    setData(data || [])
    setLoading(false)
  }
              ...
              <>
                ...
                <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                  <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', fontSize: 13, fontWeight: 600 }}>
                    자재 목록 <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 400, marginLeft: 6 }}>{data.length}건</span>
                  </div>
                  {loading ? (
                    ...
                    <div style={{ overflowX: 'auto' }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        ...
                        <tbody>{data.map((r,i) => <tr key={i}><td style={td}>{r.item_no}</td><td style={td}>{r.item_name}</td><td style={td}>{r.spec||'-'}</td><td style={td}>{r.maker||'-'}</td><td style={td}>{r.unit}</td><td style={td}><span className="badge badge-teal">{r.quantity}</span></td><td style={td}>{r.note||'-'}</td></tr>)}</tbody>
                      </table>
                    </div>
                  )}
                </div>
