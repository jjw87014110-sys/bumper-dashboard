...
export default function DashboardPage() {
  ...
  function toggleTodo(item: typeof DAILY_TODOS[0]) {
    ...
    if (!checked[item.key]) {
      // 체크 ON → 달력에 추가
      if (!todayCompleted.includes(item.label)) todayCompleted.push(item.label)
    } else {
      // 체크 OFF → 달력에서 제거
      const idx = todayCompleted.indexOf(item.label)
      if (idx > -1) todayCompleted.splice(idx, 1)
    }
    const nextCompleted = { ...savedCompleted, [todayKey]: todayCompleted }
    setCompletedDates(nextCompleted)
              ...
              <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                    <span style={{ color: doneCount === DAILY_TODOS.length ? 'var(--accent-green)' : 'var(--accent-amber)', fontWeight: 600 }}>{doneCount}</span>/{DAILY_TODOS.length}
                  </div>
                </div>
                  ...
                  {DAILY_TODOS.map(item => (
                    ...
                    >
                      <div style={{
                        ...
                        border: `2px solid ${checked[item.key] ? 'var(--accent-green)' : 'var(--border-light)'}`,
                        background: checked[item.key] ? 'var(--accent-green)' : 'transparent',
                      ...
                      <span style={{ fontSize: 12, color: checked[item.key] ? 'var(--text-muted)' : 'var(--text-primary)', textDecoration: checked[item.key] ? 'line-through' : 'none', transition: 'all 0.15s' }}>
                        ...
                        {item.regular && (
                          <span style={{ marginLeft: 6, fontSize: 9, color: 'var(--text-muted)', background: 'var(--bg-hover)', padding: '1px 5px', borderRadius: 8 }}>정기</span>
                        )}
                      </span>
                    </div>
                  ))}
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                    ...
                    >
                      ...
                      {!isWeekend && imarkingEq > 0 && (
                        <div style={{ fontSize: 9, padding: '2px 5px', borderRadius: 4, background: dayCompleted.includes('아이마킹') ? 'var(--accent-green-dim)' : 'var(--accent-teal-dim)', color: dayCompleted.includes('아이마킹') ? 'var(--accent-green)' : 'var(--accent-teal)', marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {dayCompleted.includes('아이마킹') ? '✓ ' : ''}아이마킹 #{String(imarkingEq).padStart(2,'0')}
                        </div>
                      )}
              ...
              <div style={{ padding: '10px 16px', borderTop: '1px solid var(--border)', display: 'flex', gap: 16, fontSize: 10, color: 'var(--text-muted)', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <div style={{ width: 8, height: 8, borderRadius: 2, background: 'var(--accent-teal)' }} />아이마킹 일정
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <div style={{ width: 8, height: 8, borderRadius: 2, background: 'var(--bg-hover)', border: '1px solid var(--border)' }} />정기업무
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <div style={{ width: 8, height: 8, borderRadius: 2, background: 'var(--accent-green)' }} />완료
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <div style={{ width: 8, height: 8, borderRadius: 2, background: 'var(--accent-amber)' }} />추가 일정 (클릭 삭제)
                </div>
              </div>
            </div>
          </div>
