...
export default function ConditionPage() {
  ...
  async function fetchData() {
    setLoading(true)
    const [ct, eq] = await Promise.all([
      supabase.from('condition_table').select('*, equipment(no,name,model)').order('change_date', { ascending:false }),
      ...
      <div className="main-area">
        <div className="topbar">
          <div>
            ...
            <div style={{fontSize:11,color:'var(--text-secondary)',marginTop:2}}>설비별 펀칭/융착/Air 조건값 관리</div>
          </div>
          <button className="btn btn-primary" onClick={openAdd}>+ 조건 등록</button>
        </div>
          ...
          <div className="card">
            ...
            {loading ? (
              ...
              <div style={{textAlign:'center',padding:'40px 0',color:'var(--text-muted)'}}>
                <div style={{marginBottom:12}}>등록된 조건값이 없습니다</div>
                <button className="btn btn-primary" onClick={openAdd}>+ 첫 번째 조건 등록</button>
              </div>
            ) : (
      ...
      {deleteId && (
        <div className="modal-overlay">
          <div className="modal" style={{maxWidth:360}}>
            ...
            <div style={{fontSize:13,color:'var(--text-secondary)',marginBottom:20}}>이 조건값을 삭제하시겠습니까?</div>
