...
export default function ImarkingPage() {
  ...
  const [form, setForm] = useState<any>({ change_date: new Date().toISOString().slice(0,10), category: '펀칭', mode: '', unit: '', value: '', note: '' })
  ...
  function openAdd() { setEditItem(null); setForm({ change_date: new Date().toISOString().slice(0,10), category: '펀칭', mode: '', unit: '', value: '', note: '' }); setModal(true) }
  function openEdit(r: any) { setEditItem(r); setForm({ change_date: r.change_date, category: r.category, mode: r.mode, unit: r.unit, value: r.value, note: r.note||'' }); setModal(true) }

  async function handleSave() {
    const payload = { equipment_no: selected.no, change_date: form.change_date, category: form.category, mode: form.mode, unit: form.unit, value: Number(form.value), note: form.note }
    if (editItem) {
      ...
      <div className="main-area">
        <div className="topbar">
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2 }}>설비를 선택하면 아이마킹 데이터가 표시됩니다</div>
          </div>
        </div>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {!selected ? (
              <div className="card" style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-muted)' }}>
                ...
                <div style={{ fontSize: 12 }}>왼쪽 목록에서 설비를 클릭하면 아이마킹 데이터가 표시됩니다</div>
              </div>
            ) : (
              <>
                ...
                <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                  <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', fontSize: 13, fontWeight: 600 }}>
                    아이마킹 내역 <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 400, marginLeft: 6 }}>{data.length}건</span>
                  </div>
                  {loading ? (
