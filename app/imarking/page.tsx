                  ...
                  {loading ? (
                    ...
                    <div style={{ overflowX: 'auto' }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        ...
                        <tbody>{data.map((r,i) => <tr key={i}><td style={td}>{(r.change_date||'').slice(0,10)}</td><td style={td}>{r.category||'-'}</td><td style={td}>{r.mode||'-'}</td><td style={td}>{r.unit||'-'}</td><td style={td}><span className="badge badge-blue">{r.value}</span></td><td style={td}>{r.note||'-'}</td></tr>)}</tbody>
                      </table>
                    </div>
                  )}
                </div>
